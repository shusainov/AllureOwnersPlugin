...
public class OwnersPlugin implements Aggregator {
    ...
    public void aggregate(final Configuration configuration,
        ...
        final Path dataFile = dataFolder.resolve("owners.json");
    ...
    /* default */ Tree<TestResult> getData(final List<LaunchResults> launchResults) {
        ...
        return collapseGroupsWithOnlyOneChild(owners);
    }

    protected List<TreeLayer> groupByOwners(final TestResult testResult) {
        return testResult.findOneLabel(LabelName.OWNER)
                .map(ownerName -> Arrays.asList(ownerName.split("\\.")))
    ...
    protected Tree<TestResult> collapseGroupsWithOnlyOneChild(final Tree<TestResult> owners) {
        owners.getChildren().stream()
